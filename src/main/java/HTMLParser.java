...
public class HTMLParser extends Thread {
	...
	public HTMLParser(Properties config) {
		...
		adminTableId = adminTableUri.substring(adminTableUri.indexOf("spreadsheets/d/") + 15,
				adminTableUri.indexOf("/edit"));
	...
	public void parse() {
		...
		TreeMap<String, String> categoriesTables = new TreeMap<>();
		try {
			categoriesTables = service.getCategories(adminTableId);
		} catch (IOException e) {
			connectTimeout("tableId = " + adminTableId);
			parse();
		}
		...
		for (Entry<String, String> entry : categoriesTables.entrySet()) {
	...
	private void getLinks(String category, String tableUri) {
		...
		if (!category.startsWith("http") || !tableUri.startsWith("http"))
			return;
		category = category.replace("#/", "/");
		System.out.println("\n");
		String tableId = tableUri.substring(tableUri.indexOf("/d/") + 3, tableUri.indexOf("/edit"));
		...
		if (paginationPage.last() != null) {
			...
			if (lastPage.substring(from).length() < 3) {
				last = Integer.parseInt(lastPage.substring(from));
			} else {
		...
		for (int i = 1; i <=last; i++) {
			...
			if (category.contains("?")) {
				request = String.format(category + "&p=%d", i);
			} else {
				request = String.format(category + "?p=%d", i);
			}
			...
			for (int k = 0; k < links.size(); k++) {
				...
				if (parsedRow.get(3)==null) continue;

				for (int pi = 0; pi < currentValues.size(); pi++) {
					if (currentValues.get(pi).isEmpty()) {
						currentValues.remove(pi);
						continue;
					}
					...
					// }
					if (currentValues.get(pi).get(3).equals(parsedRow.get(3))
							&& currentValues.get(pi).get(4).equals(parsedRow.get(4))) {
						ignored++;
						eq = true;
						break;
					}
	...
	private boolean clearTable(String tableId, int size) {
		try {
			...
			connectTimeout("tableId = " + tableId);
	...
	private List<Object> parse(String link) {
		...
		// row.add(doc.select(".text.text-main").text() +
		// doc.select(".single-item-header").text());
		...
		if (phoneRef != null && phoneRef.contains("phone")) {
			...
			if (phone != null) phone = convertOnlyDigit(phone).replaceFirst("7", "8");
		} else {
	...
	private Document getDocument(String link) {
		...
		try {
			document = Jsoup.connect(link)
					...
					.header("Accept", headers.getProperty("Accept")).header("Referer", headers.getProperty("Referer"))
					.header("Accept-Encoding", headers.getProperty("Accept-Encoding"))
					.header("Accept-Language", headers.getProperty("Accept-Language"))
					.header("Cookie", headers.getProperty("Cookie")).get();
		} catch (IOException e) {
			connectTimeout(link);
	...
	private String getPhone(String ref, String phoneId) {
		...
		try {
			...
			connectTimeout(uri + "/phone/" + phoneId);
	...
	private String convertDate(String rawDate) {
		...
		return rawDate.replace("Размещено ", "")
				...
				.replace("вчера",
						String.format("%02d", (yesterday.get(Calendar.DATE))) + "."
								+ String.format("%02d", (yesterday.get(Calendar.MONTH) + 1)))
				.replace(" в", "").replace(" января", ".01 ").replace(" февраля", ".02 ").replace(" марта", ".03 ")
				.replace(" апреля", ".04 ").replace(" мая", ".05 ").replace(" июня", ".06 ").replace(" июля", ".07 ")
				.replace(" августа", ".08 ").replace(" сентября", ".09 ").replace(" октября", ".10 ")
				.replace(" ноября", ".11 ").replace(" декабря", ".12 ").trim();
	}
	...
	private void connectTimeout(String uri) {
		for (int i = 30; i >= 0; i--) {
			System.out.print("\r");
			System.out.print("Could not connect to " + uri + " Retrying through ");
			System.out.print(i);
